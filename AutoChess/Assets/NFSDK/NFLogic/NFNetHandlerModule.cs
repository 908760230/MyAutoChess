    ...
    public partial class NFNetHandlerModule : NFIModule
    {
        ...
        private void EGMI_ACK_OBJECT_ENTRY(int id, MemoryStream stream)
        {
            ...
            for (int i = 0; i < xData.ObjectList.Count; ++i)
            {
                ...
                {
                    Debug.Log(xObjectID.ToString() + " has already existed!");
                    NFIProperty pos = xGO.FindProperty(NFrame.Player.Position);
                    pos.SetVector3(vPos);
                    continue;
                }
        ...
        private void EGMI_ACK_OBJECT_PROPERTY_ENTRY(int id, MemoryStream stream)
        {
            ...
            for (int i = 0; i < xData.MultiPlayerProperty.Count; i++)
            {
                ...
                if (mxObjectDataBuff.TryGetValue(xObjectID, out xDataBuff))
                {
                    // 将玩家的 property 和 record 赋给 client 玩家的databuf
        ...
        private void AttachObjectData(NFGUID self)
        {
            ...
            if (mxObjectDataBuff.TryGetValue(self, out xDataBuff))
            {
                ...
                {
                    /// 
                    Debug.Log("player id from server is : " + mHelpModule.PBToNF(xDataBuff.xPropertyList.PlayerId));
                    NFIObject go = mKernelModule.GetObject(mHelpModule.PBToNF(xDataBuff.xPropertyList.PlayerId));
                    if (go == null) Debug.Log("错误在的地方：" + self.ToString());
        ...
        private void EGMI_ACK_DATA_FINISHED(int id, MemoryStream stream)
        {
            ...
            for (int i = 0; i < xData.ObjectList.Count; ++i)
            {
                ...
                Debug.Log("ack data finished create Object: " + strClassName + " " + xObjectID.ToString() + " " + strConfigID + " (" + xInfo.X + "," + xInfo.Y + "," + xInfo.Z + ")");
                //if (strClassName == "ChessPlane") mSceneModule.battlePlaneId = xObjectID;
                ...
                {   
                    ...
                    if (null == xGO)
                    {
                        Debug.LogError("object ID not be created: " + xObjectID.ToString() + "  ConfigID: " + strClassName);
                        continue;
                    }
                }
            }
